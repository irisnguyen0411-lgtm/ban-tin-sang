...
def collect():
    ...
    for si, section in enumerate(SECTIONS):
        ...
        # Nguồn tạp chí (Tatler, BrandsVietnam, Vietcetera) đăng thưa hơn báo
        # ngày nên được giữ chỗ tối đa 4 tin/mục để không bị tin nóng trong
        # ngày đẩy ra ngoài
        for rsv_src in ("Tatler", "BrandsVietnam", "Vietcetera"):
            ...
            for a in extra[:max(0, 4 - have)]:
                ...
                for i in range(len(selected) - 1, -1, -1):
                    if selected[i]["source"] not in ("Tatler", "BrandsVietnam", "Vietcetera"):
                        selected[i] = a
                        break
