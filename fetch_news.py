...
SECTIONS = [
    ...
    },
    {
        ...
        "feeds": [
            ...
            ("Vietcetera", "scrape:vietcetera-cuoc-song"),
    ...
    {
        "id": "hr-insights",
        "title": "Kiến thức & Xu hướng Nhân sự",
        "feeds": [
            ("HR Insights", "static:hr-reports"),
        ],
    },
]
...
# Tiêu đề dạng "thẻ nhân vật" (Ông/Bà/Tiến sĩ ... + chức danh, không phải tin
# tức thật sự) — loại khỏi bản tin vì không phải nội dung thời sự.
PERSON_PROFILE_RE = re.compile(
    r"^(Ông|Bà|Tiến sĩ|TS\.|Thạc sĩ|ThS\.|PGS\.|GS\.|PGS\.TS|GS\.TS)\s",
    re.IGNORECASE,
)


def is_person_profile(title: str) -> bool:
    title = (title or "").strip()
    if not PERSON_PROFILE_RE.match(title):
        return False
...
TITLE_NORM_RE = re.compile(r"[^0-9a-zàáâãèéêìíòóôõùúăđĩũơưạảấầẩẫậắằẳẵặẹẻẽ"
                            r"ếềểễệỉịọỏốồổỗộớờởỡợụủứừửữựỳỵỷỹ ]+")


def normalize_title(title: str) -> str:
    """Chuẩn hoá tiêu đề để phát hiện tin trùng lặp giữa các chuyên mục
    (cùng một bài được nhiều báo chị em đăng lại với link khác nhau)."""
    t = (title or "").lower().strip()
    t = TITLE_NORM_RE.sub(" ", t)
...
def scrape_brandsvietnam():
    """BrandsVietnam không có RSS — đọc bài từ trang Cộng đồng (nhiều chủ đề:
    tech talks, brand collab, thị trường/giải trí...) thay vì chỉ trang Tiêu điểm."""
    resp = requests.get("https://www.brandsvietnam.com/community/", timeout=TIMEOUT,
                        headers={"User-Agent": UA})
    items, seen = [], set()
    for m in re.finditer(
            r'<a[^>]+href="(https://www\.brandsvietnam\.com/congdong/topic/[^"]+)"[^>]*>([^<]{10,200})</a>',
            resp.text):
...
def scrape_vietcetera_category(slug: str):
    """Vietcetera (giao diện mới) render danh sách bài qua __NEXT_DATA__ (JSON)
    thay vì RSS. Đọc chuyên mục /vn/chuyen-muc/{slug} (vd: cuoc-song) để lấy
    thêm bài lifestyle/đời sống."""
    url = f"https://vietcetera.com/vn/chuyen-muc/{slug}"
    ...
    try:
        ...
        arts = data["props"]["pageProps"]["arrFeatureArticle"]
    ...
    for a in arts:
        ...
        if not title or not slug_a:
            continue
        items.append({
            "title": title,
            "link": f"https://vietcetera.com/vn/{slug_a}",
...
HR_REPORTS = [
    {
        "title": "Deloitte: Các tổ chức chiến thắng sẽ xây dựng “lợi thế con người”",
        "source": "Deloitte",
        "link": "https://www.deloitte.com/us/en/about/press-room/deloitte-report-winning-organizations-will-build-the-human-advantage.html",
        "summary": (
            "Thông cáo giới thiệu báo cáo Global Human Capital Trends của Deloitte, "
            "lập luận rằng lợi thế cạnh tranh trong kỷ nguyên AI không đến từ công "
            "nghệ đơn thuần mà từ cách tổ chức phát huy năng lực con người — kết "
            "hợp kỹ năng, văn hoá và mô hình làm việc linh hoạt."
        ),
        "days_ago": 210,
    },
    {
        "title": "Deloitte: Toàn cảnh Global Human Capital Trends",
        "source": "Deloitte",
        "link": "https://www.deloitte.com/ch/en/services/consulting/perspectives/global-human-capital-trends.html",
        "summary": (
            "Trang tổng hợp chuỗi nghiên cứu thường niên của Deloitte về xu hướng "
            "vốn nhân lực toàn cầu — nơi HR lãnh đạo có thể tra cứu các chủ đề như "
            "tái thiết kế công việc, năng lực thích ứng và vai trò của AI trong "
            "quản trị nhân sự qua từng năm."
        ),
        "days_ago": 205,
    },
    {
        "title": "Deloitte 2026: Xây dựng lực lượng lao động thích ứng",
        "source": "Deloitte",
        "link": "https://www.deloitte.com/us/en/insights/topics/talent/human-capital-trends/2026/creating-an-adaptable-workforce.html",
        "summary": (
            "Một phần báo cáo Human Capital Trends 2026, phân tích vì sao khả năng "
            "thích ứng nhanh của nhân sự — thay vì chỉ tối ưu hiệu suất — đang trở "
            "thành năng lực cạnh tranh cốt lõi, và gợi ý cách HR tái cấu trúc kỹ "
            "năng, vai trò để bắt kịp tốc độ thay đổi công nghệ."
        ),
        "days_ago": 200,
    },
    {
        "title": "Deloitte 2026: Hiện đại hoá các chức năng doanh nghiệp",
        "source": "Deloitte",
        "link": "https://www.deloitte.com/us/en/insights/topics/talent/human-capital-trends/2026/modernizing-corporate-functions.html",
        "summary": (
            "Phần tiếp theo của Human Capital Trends 2026 bàn về việc các phòng ban "
            "hỗ trợ (HR, tài chính, vận hành) cần được thiết kế lại quanh dữ liệu "
            "và AI ra sao để không chỉ vận hành hiệu quả mà còn tạo giá trị chiến "
            "lược cho tổ chức."
        ),
        "days_ago": 195,
    },
    {
        "title": "WEF: Báo cáo Chief People Officers Outlook 2026",
        "source": "World Economic Forum",
        "link": "https://www.weforum.org/publications/chief-people-officers-outlook-may-2026/",
        "summary": (
            "Khảo sát các Giám đốc Nhân sự (CHRO) toàn cầu của WEF, phản ánh những "
            "ưu tiên hàng đầu năm 2026: nâng cấp kỹ năng lực lượng lao động trước "
            "làn sóng AI, giữ chân nhân tài và tái định hình vai trò lãnh đạo nhân "
            "sự trong bối cảnh bất định."
        ),
        "days_ago": 60,
    },
    {
        "title": "WEF: AI đang thay đổi kỹ năng lãnh đạo như thế nào",
        "source": "World Economic Forum",
        "link": "https://www.weforum.org/stories/artificial-intelligence/how-is-ai-changing-the-skills-for-leadership-and-how-should-organizations-prepare/",
        "summary": (
            "Bài phân tích của WEF về cách AI định hình lại bộ kỹ năng lãnh đạo cần "
            "thiết — từ tư duy phản biện, ra quyết định dựa trên dữ liệu đến năng "
            "lực dẫn dắt đội ngũ con người — và khuyến nghị để tổ chức chuẩn bị đội "
            "ngũ lãnh đạo kế cận phù hợp."
        ),
        "days_ago": 45,
    },
]
...
def static_hr_reports():
    """Danh sách báo cáo/kiến thức HR chọn lọc từ các hãng tư vấn uy tín
    (Deloitte, World Economic Forum...). Không có RSS nên cập nhật thủ công
    trong mã nguồn; ngày đăng là ước lượng để sắp xếp thứ tự hiển thị."""
    now = datetime.now(VN_TZ)
    items = []
    for rep in HR_REPORTS:
        items.append({
            "title": rep["title"],
            "link": rep["link"],
            "summary": rep["summary"],
            "_source_override": rep["source"],
            "_dt_override": now - timedelta(days=rep["days_ago"]),
        })
    return items


SCRAPERS = {
    "scrape:brandsvietnam": scrape_brandsvietnam,
    "scrape:vietcetera-cuoc-song": lambda: scrape_vietcetera_category("cuoc-song"),
    "static:hr-reports": static_hr_reports,
}
...
def collect():
    ...
    for si, section in enumerate(SECTIONS):
        ...
        for source, url in section["feeds"]:
            ...
            for e in entries:
                ...
                if title_key and title_key in seen_titles:
                    continue
                if is_person_profile(title):
                    continue
                summary = truncate(strip_html(e.get("summary") or e.get("description") or ""))
                if is_blocked(title, summary):
                    continue
                dt = e.get("_dt_override") or entry_datetime(e)
                articles.append({
                    "source": e.get("_source_override") or src,
                    "sec_id": section["id"],
        ...
        # Nguồn tạp chí (BrandsVietnam, Vietcetera) đăng thưa hơn báo ngày nên
        # được giữ chỗ tối đa 4 tin/mục để không bị tin nóng trong ngày đẩy ra
        # ngoài
        for rsv_src in ("BrandsVietnam", "Vietcetera"):
            ...
            for a in extra[:max(0, 4 - have)]:
                if len(selected) < MAX_PER_SECTION:
                    selected.append(a)
                    continue
                for i in range(len(selected) - 1, -1, -1):
                    if selected[i]["source"] not in ("BrandsVietnam", "Vietcetera"):
                        selected[i] = a
                        break
...
<header class="masthead">
  <div class="wrap">
    ...
    <div class="meta">
      ...
      <a class="refresh-btn" href="https://github.com/irisnguyen0411-lgtm/ban-tin-sang/actions/workflows/update.yml" target="_blank" rel="noopener" title="Mở GitHub → bấm Run workflow → chờ 30 giây → tải lại trang này">⟳ Cập nhật tin ngay</a>
    </div>
  </div>
</header>
...
<footer>
  <div class="wrap">
    <h4>Nguồn tin</h4>
    <p>VnExpress · Dân Trí · CafeF · CafeBiz · Znews · Thanh Niên · Tuổi Trẻ · VietNamNet · Người Lao Động · VnEconomy · Vietcetera · BrandsVietnam · Deloitte · World Economic Forum</p>
    <details>
